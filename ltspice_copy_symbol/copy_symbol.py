...
Parser = argparse.ArgumentParser(
    description="Simple script that copies a default LTspice symbol to a custom-made one."
)
Parser.add_argument(
    "-c",
    "--component",
...
def copy_symbol_main(component: str, target_file: str):
    ...
    for i, comp in enumerate(FILE_POSITIONS):
        if not os.path.isfile(LTSPICE_DEFAULT_SYMBOL_FOLDER + comp):
            print(
                f'Component "{ACCEPTED_COMPONENTS[i]}" with filename "{comp}" is missing from the default LTspice symbol folder "{LTSPICE_DEFAULT_SYMBOL_FOLDER}"!'
            )
            error_found = True
    ...
    component_filename = (
        LTSPICE_DEFAULT_SYMBOL_FOLDER
        + FILE_POSITIONS[ACCEPTED_COMPONENTS.index(component)]
    )
    ...
    try:
        ...
        print(
            f'Component {component} is not in the default LTspice symbol folder "{LTSPICE_DEFAULT_SYMBOL_FOLDER}"!'
        )
        exit(3)
